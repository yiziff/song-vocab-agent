...
const GLOSSARY_PATH = path.join(ROOT, "data", "cet6_glossary.json");
...
async function callDeepSeek(cfg, ctx) {
  const system = `你是英语学习助手。根据「歌手、歌名、目标词、英文歌词行、词典义」用中文做简短讲解。
要求：
1. 返回严格 JSON 对象，键为 line_zh、story，不要 markdown。
2. line_zh：该英文歌词行的通顺中文翻译（若已有译文可润色，不要空）。
3. story：2～4 句中文。先点明这句在歌里的场景，再解释目标词在此语境的用意（深度语义缝合）。
4. 不编造与歌词无关的情节；不确定就写「从这句字面看…」。
5. 不要复述整首歌剧情，聚焦这一行与这个词。`;
