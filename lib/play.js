...
<head>
  ...
  <style>
    ...
    *::-webkit-scrollbar-corner {
    ...
    :root {
      --glow-a: rgba(91, 192, 190, 0.45);
      --glow-b: rgba(58, 80, 107, 0.5);
      --glow-c: rgba(28, 37, 65, 0.55);
      --beat-scale: 1;
      --beat-glow: 0.35;
    }
    #cover-ambient {
      ...
      filter: blur(72px) saturate(1.75) brightness(0.88);
      transform: scale(1.12);
      opacity: 0.92;
      transition: background-image .45s ease, opacity .45s ease;
    }
    #cover-stage.has-cover #cover-ambient {
    ...
    #cover-stage.has-cover #cover-bg { opacity: 0.42; }
    #cover-glow {
      position: absolute; inset: 0;
      background:
        radial-gradient(ellipse 75% 55% at 18% 28%, var(--glow-a), transparent 58%),
        radial-gradient(ellipse 65% 50% at 82% 72%, var(--glow-b), transparent 55%),
        radial-gradient(ellipse 55% 40% at 50% 100%, var(--glow-c), transparent 50%);
      opacity: 0.5;
      mix-blend-mode: screen;
      transition: opacity .4s ease;
    }
    #cover-stage.has-cover #cover-glow {
      animation: ambient-breathe 11s ease-in-out infinite reverse;
    }
    #cover-dim {
      position: absolute; inset: 0;
      background:
        radial-gradient(ellipse 70% 60% at 50% 40%, rgba(0,0,0,.18), rgba(0,0,0,.52) 75%),
    ...
    body.is-audio-playing #cover-glow {
      opacity: calc(0.42 + var(--beat-glow) * 0.55);
    }
    body.is-audio-playing #cover-ambient {
      filter: blur(68px) saturate(1.95) brightness(0.95);
    }
    .macos-window {
    ...
    .player-card.is-playing {
      box-shadow:
        ...
        0 0 calc(28px + var(--beat-glow) * 36px) color-mix(in srgb, var(--glow-a) 55%, transparent),
        inset 0 1px 0 rgba(255,255,255,.55);
    }
    .player-card.is-playing #cover-thumb {
      box-shadow:
        0 4px 14px rgba(0,0,0,.18),
        0 0 calc(10px + var(--beat-glow) * 18px) color-mix(in srgb, var(--glow-a) 70%, transparent);
    }
    .player-card.is-playing #seek-range::-webkit-slider-runnable-track {
      background: color-mix(in srgb, var(--glow-a) 45%, rgba(0,0,0,.1));
      box-shadow: 0 0 calc(6px + var(--beat-glow) * 10px) color-mix(in srgb, var(--glow-a) 50%, transparent);
    }
    .player-card.is-playing #seek-range::-moz-range-track {
      background: color-mix(in srgb, var(--glow-a) 45%, rgba(0,0,0,.1));
    }
    .player-card.is-playing #seek-range::-webkit-slider-thumb {
      transform: scale(calc(0.95 + var(--beat-glow) * 0.2));
      box-shadow: 0 1px 4px rgba(0,0,0,.25), 0 0 10px color-mix(in srgb, var(--glow-a) 60%, transparent);
    }
    .player-card-top {
    ...
    @media (prefers-reduced-motion: reduce) {
      #cover-stage.has-cover #cover-ambient,
      #cover-stage.has-cover #cover-glow { animation: none !important; }
      .player-card { transform: none !important; will-change: auto; }
      .player-card.is-playing #seek-range::-webkit-slider-thumb { transform: none; }
    }
  </style>
</head>
<body class="desktop">
  <div id="cover-stage" aria-hidden="true">
    <div id="cover-ambient"></div>
    <div id="cover-bg"></div>
    <div id="cover-glow"></div>
    <div id="cover-dim"></div>
  </div>
  ...
  <script>
    ...
    const el = {
      ...
      coverAmbient: document.getElementById('cover-ambient'),
    ...
    function clearCover() {
      lastCoverSongId = '';
      paletteSeq += 1;
      const layers = [el.coverAmbient, el.coverBg];
      for (const node of layers) {
        if (!node) continue;
        node.style.backgroundImage = '';
      }
      if (el.coverStage) el.coverStage.classList.remove('has-cover');
      if (el.coverBg) el.coverBg.classList.remove('has-cover');
      if (el.coverThumb) {
      ...
      document.documentElement.style.setProperty('--glow-a', 'rgba(91, 192, 190, 0.45)');
      document.documentElement.style.setProperty('--glow-b', 'rgba(58, 80, 107, 0.5)');
      document.documentElement.style.setProperty('--glow-c', 'rgba(28, 37, 65, 0.55)');
    }

    function rgbaFromAvg(bucket, alpha) {
    ...
    async function extractCoverPalette(songId) {
      if (!API || !songId) return;
      const seq = ++paletteSeq;
      try {
        const img = new Image();
        img.decoding = 'async';
        img.src = API + '/api/cover-image?song_id=' + encodeURIComponent(songId);
        await img.decode();
        ...
        for (let i = 0; i < data.length; i += 4) {
          ...
          if (max < 36 || min > 228) continue;
          const sat = max - min;
          if (sat < 18 && max < 160) continue;
        ...
        if (!top.length || seq !== paletteSeq) return;
        document.documentElement.style.setProperty('--glow-a', rgbaFromAvg(top[0], 0.58));
        document.documentElement.style.setProperty(
          '--glow-b',
          rgbaFromAvg(top[1] || top[0], 0.5)
        );
        document.documentElement.style.setProperty(
          '--glow-c',
          rgbaFromAvg(top[2] || top[1] || top[0], 0.46)
        );
      } catch (_) {
        /* CORS / offline: keep ambient blur only */
      }
    }

    function applyCoverUrl(url, songId) {
      ...
      const bg = 'url("' + safe + '")';
      if (el.coverAmbient) el.coverAmbient.style.backgroundImage = bg;
      if (el.coverBg) {
        el.coverBg.style.backgroundImage = bg;
        el.coverBg.classList.add('has-cover');
      }
      if (el.coverStage) el.coverStage.classList.add('has-cover');
      if (el.coverThumb) {
        el.coverThumb.style.backgroundImage = bg;
      ...
      if (songId) extractCoverPalette(songId);
    }
    ...
    async function setCoverForSong(songId) {
      ...
      if (id === lastCoverSongId && el.coverStage?.classList.contains('has-cover')) return;
      if (!API) {
        clearCover();
        return;
      }
      ...
      try {
        ...
        if (data.ok && data.cover_url) {
          lastCoverSongId = id;
          applyCoverUrl(data.cover_url, id);
        } else {
          clearCover();
        }
      } catch {
    ...
    function stopBeatPulse() {
      ...
      document.documentElement.style.setProperty('--beat-glow', '0.35');
      if (el.audioWrap) el.audioWrap.classList.remove('is-playing');
    }

    function startBeatPulse() {
      stopBeatPulse();
      if (reduceMotion || !el.audio || el.audio.paused) return;
      document.body.classList.add('is-audio-playing');
      if (el.audioWrap) el.audioWrap.classList.add('is-playing');
      ...
      const tick = (now) => {
        ...
        const t = (now - t0) / 1000;
        const beat = 0.5 + 0.5 * Math.sin(t * Math.PI * 2 * 1.12);
        const swell = 0.5 + 0.5 * Math.sin(t * Math.PI * 2 * 0.28);
        const amp = 0.008 + beat * 0.015 * (0.55 + 0.45 * swell);
        document.documentElement.style.setProperty('--beat-scale', String(1 + amp));
        document.documentElement.style.setProperty('--beat-glow', String(0.28 + beat * 0.4));
