...
<head>
  ...
  <style>
    ...
    *::-webkit-scrollbar-corner {
    ...
    .player-card {
      margin: 1.15rem 0 0; padding: 1rem;
      border-radius: 12px;
    ...
    .quiz { display: flex; gap: .75rem; margin-top: 1.35rem; }
    .quiz button {
...
<body class="desktop">
  ...
  <script>
    ...
    if (el.seekRange) {
      ...
      el.seekRange.addEventListener('pointerup', () => { scrubbing = false; syncScrubber(); });
      ...
      el.seekRange.addEventListener('change', () => {
        ...
        if (!Number.isFinite(dur) || dur <= 0) return;
        const ratio = Number(el.seekRange.value) / 1000;
        try { el.audio.currentTime = ratio * dur; } catch (_) {}
        highlightLyricAt(el.audio.currentTime * 1000);
        scrubbing = false;
        syncScrubber();
      });
